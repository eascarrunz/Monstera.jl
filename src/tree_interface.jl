#=
This file defines the interface for tree (and node and branch) types.
...
"""
Base.size(tree::AbstractTree) = size(tree)
...
Resize a tree to contain `n` nodes and `b` branches. 
"""
Base.resize!(::AbstractTree, n, b) = resize!(tree, n, b)
Base.resize!(tree::AbstractTree, n) = resize!(tree, n, n - 1)


"""
    reindex!(tree)

Re-assign id numbers to the branches and nodes in a tree, in preorder, and remove branches
and nodes that are not descendants of the root of the tree.
"""
reindex!(tree::AbstractTree) = reindex!(tree)
...
function swapnodes!(
    ...
    # unlink!(branch1, node1)
    # unlink!(branch1, pnode1)
    # unlink!(branch2, node2)
    # unlink!(branch2, pnode2)
    unlink!(pnode1, node1)
    unlink!(pnode2, node2)
